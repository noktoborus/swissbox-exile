...
BEGIN
	...
	EXCEPTION
		...
		/* FIXME: непонятно что делать с чанками, совпадающие по chunk_guid и
			разными hash. В теории такая ситуация возможна,
			но на практике маловероятна. Если возникнет, то загрузка
			второго чанка не произойдёт и будет отправлено сообщение что
			чанк уже есть.d

			В общем-то, учёт чанков идёт по guid и проблемы одинаковых guid
			касаются только клиента, но всё же.
		*/
